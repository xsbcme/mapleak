<template>
  <Teleport to="body">
    ...
    <div :class="fullscreen ? 'source-viewer-fullscreen' : 'source-viewer-modal'" style="background: var(--c-bg)">
      <!-- ── 顶栏 ──────────────────────────────────────────────── -->
      <div
        class="flex items-center gap-2 px-3 shrink-0 border-b"
      ...
      >
        ...
        <template v-if="isProjectMode">
          ...
          <span
            v-if="data"
            class="hidden sm:block text-[10px] px-2 py-0.5 rounded shrink-0"
          ...
          <!-- 是否包含第三方依赖切换 -->
          <button
            class="sv-btn shrink-0 text-[10px] px-2 py-0.5 rounded"
            :class="includeNodeModules ? 'sv-btn-active' : ''"
            :title="t('sv.includeDepsTooltip')"
            @click="
        ...
        <button
          class="sv-btn"
          :class="{ 'sv-btn-active': fullscreen }"
          :title="fullscreen ? t('sv.fullscreenOff') : t('sv.fullscreenOn')"
      ...
      <div v-else-if="data" class="flex" style="flex: 1; min-height: 0; overflow: hidden">
        ...
        >
          ...
          <template v-else>
            ...
            <div
              v-if="isProjectMode && props.finding"
            ...
            <div
              class="flex items-center gap-1 px-1.5 shrink-0 border-b"
              style="height: 34px; border-color: var(--c-border); background: var(--c-surface)"
            >
              <span
                class="text-[10px] uppercase tracking-wide font-bold flex-1 truncate ml-0.5"
                style="color: var(--c-muted)"
              >
                {{
                  fileSearch
              ...
              <template v-if="!fileSearch">
                <button
                  ...
                  :class="{ 'sv-btn-active': viewMode === 'flat' }"
                  :title="t('sv.flatList')"
                >
                  &#8801;
                </button>
                <button
                  @click="viewMode = 'tree'"
                  class="sv-btn sv-btn-icon"
                  :class="{ 'sv-btn-active': viewMode === 'tree' }"
                  :title="t('sv.treeView')"
                >
                  &#8862;
                </button>
            ...
            <!-- 搜索框 -->
            <div
              class="px-2 py-1.5 shrink-0 border-b"
            ...
            <div ref="sidebarEl" class="overflow-y-auto flex-1" @scroll.passive="onSidebarScroll">
              ...
              <template v-if="fileSearch || viewMode === 'flat'">
                <div :style="'position:relative; height:' + sidebarVirtual.totalH + 'px'">
                  ...
                  <button
                    v-for="file in sidebarVirtual.items"
                    :key="file.origIdx"
                    @click="openFile(file.origIdx)"
                    class="w-full text-left px-3 flex items-center gap-1.5 transition-colors"
                    :style="
                      activeTab?.fileIdx === file.origIdx
                  ...
                  >
                    ...
                    <span
                      class="flex-1 truncate text-[11px] font-mono min-w-0"
                    ...
                    <span
                      v-if="fileSearch"
                      class="shrink-0 text-[9px] font-mono truncate max-w-[80px]"
                    ...
                    <!-- 工程模式：来源 .map -->
                    <span
                      v-else-if="isProjectMode && file.mapFile"
                      class="shrink-0 text-[9px] font-mono truncate max-w-[70px]"
                      style="color: var(--c-border2)"
                      :title="file.mapFile"
                    >
              ...
              <template v-else>
                <template v-for="node in visibleTreeNodes" :key="node.key">
                  ...
                  <button
                    ...
                    :title="node.fullPath"
                    :style="
                      'height:28px; border-bottom:1px solid rgba(30,58,98,.25); padding-left:' +
                  ...
                  <button
                    v-else
                    @click="openFile(node.fileIdx)"
                    class="w-full text-left flex items-center gap-1.5 transition-colors"
                    :title="node.fullPath"
                    :style="
                      activeTab?.fileIdx === node.fileIdx
              ...
              <button
                v-if="readmeFetching || readmeHtml"
                @click="openReadme"
                class="w-full text-left px-3 flex items-center gap-1.5 transition-colors"
                :style="
                  isReadmeActive
              ...
              >
                ...
                <span
                  class="flex-1 truncate text-[11px] font-mono"
                  :style="isReadmeActive ? 'color:var(--c-accent)' : 'color:var(--c-text2)'"
                  >README.md</span
                >
                <svg
                  v-if="readmeFetching"
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <!-- 代码区标题栏 -->
          <div
            v-if="!isReadmeActive && activeFile"
            class="flex items-center gap-2 px-3 shrink-0 border-b"
            style="height: 34px; border-color: var(--c-border); background: var(--c-surface)"
          >
            <!-- 语言徽章 -->
            <span
              class="text-[9px] px-1.5 py-0.5 rounded font-mono font-bold shrink-0"
              :style="
                'background:' +
            ...
            <!-- 工程模式：来源 .map 文件 -->
            <span
              v-if="isProjectMode && activeFile.mapFile"
              ...
              >
                <tbody>
                  <tr v-for="li in visibleLineIndices" :key="li" class="hover:bg-white/[.025]">
                    <!-- 行号 -->
                    <td
                      class="select-none text-right pr-3 pl-2 sticky left-0"
                      style="
                        width: 52px;
                        min-width: 52px;
                        color: var(--c-border2);
          ...
          <div
            v-if="truncated"
            class="shrink-0 text-center text-[10px] py-1"
      ...
      <!-- ── 无内容 ──────────────────────────────────────────────── -->
      <div v-else class="flex-1 flex items-center justify-center text-xs" style="color: var(--c-muted)">
        {{ t("sv.noContent") }}
      </div>
    </div>
  </Teleport>
...
import { fmtBytes, baseName, fileExt, normPath, parentPath } from "../utils/format.js";
